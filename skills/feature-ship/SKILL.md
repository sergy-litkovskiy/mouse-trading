...
`--no-deps` keeps Postgres down where it is not needed.
...
| `deps:check`: `ports-and-domain-have-no-io` | `typeorm`/`pg`/`fastify` in a port or use-case, `import type` included |
| `tsc` on `enum` / `constructor(private ...)` | non-erasable syntax under `erasableSyntaxOnly` |
